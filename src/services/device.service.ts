...
export const addDevice = async (device: Device): Promise<any | undefined> => {
	try {
		const payload = {
			...
			MeterTypeId: device.meterTypeId != null ? Number(device.meterTypeId) : null,
	...
	try {
		...
		if (!data?.status) {
			return {
				...
				message:
					data?.errors?.[0] || data?.message || data?.data?.message || `Failed to trigger sync for device ${deviceId}.`,
			};
		}
		...
		return {
			...
			statusCode: data?.statusCode,
			data: data?.data,
			message: data?.data?.message || data?.message || `Sync request submitted for device ${deviceId}.`,
		};
	} catch (error: any) {
...
export const saveDeviceConfiguration = async (id: string | number, parameterIds: number[]): Promise<any | null> => {
	try {
		const { data } = await apiClient.post(`/device/${id}/configuration`, {
			parameterIds,
		});
