...
export const fetchAllDeviceSchedules = async (): Promise<DeviceScheduleItem[]> => {
    try {
        const { data } = await apiClient.get<ApiResponse<DeviceScheduleItem[]>>(
            '/device/schedules'
        );
        return data.data ?? [];
    } catch (error) {
...
export const fetchDeviceSchedule = async (
    id: number
): Promise<DeviceScheduleItem | null> => {
    try {
        const { data } = await apiClient.get<ApiResponse<DeviceScheduleItem>>(
            `/device/schedule/${id}`
        );
