...
export function DeviceRTable({
	...
	return (
		<Card sx={{ maxWidth: "1400px", width: "100%", borderRadius: "8px" }}>
			<Box
				sx={{ p: 2, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 1 }}
			>
				...
				<Table size="small">
					<TableHead>
						<TableRow sx={{ bgcolor: "action.hover" }}>
							...
							<TableCell align="right" sx={{ fontWeight: 700, py: 1.2, px: 2 }}>
								Live Value
							</TableCell>
					...
					<TableBody>
						{items.map((item, index) => {
							...
							return (
								<TableRow
									key={item.parameterId || item.obisCode || index}
									hover
									sx={{ bgcolor: isEven ? "background.paper" : "action.hover" }}
								>
									<TableCell sx={{ py: 1, px: 2, fontWeight: 600, fontSize: "0.8125rem" }}>
										{item.parameterName}
									</TableCell>
									<TableCell
										sx={{ py: 1, px: 2, fontSize: "0.75rem", fontFamily: "monospace", color: "text.secondary" }}
									>
										{item.obisCode}
									</TableCell>
									<TableCell
			...
			<Box sx={{ p: 1.5, px: 2, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
				<Typography variant="caption" color="text.secondary">
					Total live parameters scanned: {items.length}
				</Typography>
			</Box>
		</Card>
	);
