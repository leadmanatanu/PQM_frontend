...
export function EventStatusCheckboxCard({
	...
	if (!hubConnectionRef.current) {
		...
		const hubUrl = `${serverUrl}/hubs/meter`;
		hubConnectionRef.current = new HubConnectionBuilder().withUrl(hubUrl).withAutomaticReconnect().build();
	}
	...
	useEffect(() => {
		...
		const startHubConnection = async () => {
			try {
				...
				console.error(
					"[SignalR] Connection to MeterHub failed. Please check if the backend hub at http://localhost/hubs/meter is running and CORS is configured.",
					err
				);
			}
		};
			...
			<Card>
				<CardContent>
					<Alert severity="warning">
						Event parameters were not found for OBIS Code {obisCode}. Ensure the device is connected and parameter scan
						has run.
					</Alert>
				</CardContent>
			</Card>
			...
			<Card>
				...
				<CardContent>
					{rows.length > 0 ? (
						<TableContainer component={Paper} sx={{ maxHeight: 500 }}>
							<Table stickyHeader size="small">
								<TableHead>
									<TableRow>
										{columns.map((col) => (
											<TableCell key={col} sx={{ fontWeight: "bold", bgcolor: "background.neutral" }}>
												{col}
											</TableCell>
										))}
									</TableRow>
						...
						<Alert severity="info">No event occurrences have been logged for this event category.</Alert>
					)}
				</CardContent>
			</Card>
		</Stack>
