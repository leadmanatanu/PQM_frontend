...
import { addDevice, editDevice, fetchMeterTypes } from "../../../api/device";
import type { Device } from "../../../components/dashboard/device/devices-table";
import { DeviceScheduleItem, fetchAllDeviceSchedules } from "../../../services/schedule.service";
...
export function AddDeviceForm({
	...
	const [meterTypes, setMeterTypes] = React.useState<MeterTypeItem[]>([]);

	const [selectedMeterTypeId, setSelectedMeterTypeId] = React.useState<number | null>(null);
	...
	React.useEffect(() => {
		if (editingDevice) {
			...
			setTxtPort(editingDevice.port !== undefined && editingDevice.port !== null ? String(editingDevice.port) : "");

			setSelectedValue(editingDevice.isActive ? "1" : "0");
			...
			setSelectedMeterTypeId(editingDevice.meterTypeId ?? null);

			setTxtTimeZoneId(editingDevice.timeZoneId || "India Standard Time");

			// ONE device = ONE schedule.
			setSelectedScheduleId(editingDevice.deviceSyncScheduleId ?? null);
		} else {
			resetForm();
		}
	}, [editingDevice]);
	...
	const handleMeterTypeChange = (event: SelectChangeEvent<number>) => {
		const value = Number(event.target.value);

		setSelectedMeterTypeId(value === 0 ? null : value);
	...
	const handleScheduleChange = (event: SelectChangeEvent<number>) => {
		...
		setSelectedScheduleId(value === 0 ? null : value);
	...
	const validateForm = (): boolean => {
		...
		if (selectedMeterTypeId === null || selectedMeterTypeId === undefined) {
	...
	const handleSubmit = async (e: React.MouseEvent<HTMLButtonElement>) => {
		...
		const device: Device = {
			...
			// ONE device = ONE schedule.
			deviceSyncScheduleId: selectedScheduleId,
		};
					...
					<Grid container spacing={1.5}>
						...
						>
							<FormControl fullWidth size="small" error={!!errors.name}>
								...
								<OutlinedInput label="Device Name" value={txtName} onChange={handleNameChange} />
						...
						>
							<FormControl fullWidth size="small" error={!!errors.meterType}>
								...
								{errors.meterType && <FormHelperText>{errors.meterType}</FormHelperText>}
							</FormControl>
						</Grid>
						...
						>
							<FormControl fullWidth size="small" error={!!errors.ip}>
								...
								<OutlinedInput label="IP Address" name="ip" type="text" value={txtIP} onChange={handleIPChange} />
